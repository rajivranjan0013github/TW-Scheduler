...
export const Header = ({ selectedAccounts, setSelectedAccounts }) => {
  ...
  return (
    <header className="h-16 border-b border-white/10 bg-black px-3 flex items-center justify-between sticky top-0 z-30 text-white">
      ...
      <div className="relative">
        <button
          ...
          className="flex items-center gap-2 bg-[#0a0a0a] hover:bg-white/10 px-3.5 py-1.5 rounded-lg text-xs font-medium border border-white/10 transition-all text-white"
        >
        ...
        {showAccountDropdown && (
          <div className="absolute left-0 mt-2 w-72 bg-black border border-white/10 rounded-xl shadow-2xl shadow-black/80 p-3 z-50 animate-in fade-in duration-150 text-white">
            <div className="flex justify-between items-center pb-2 border-b border-white/10 mb-2">
              ...
              <div className="flex gap-2">
                <button onClick={selectAllAccounts} className="text-[10px] text-zinc-400 hover:text-white">All</button>
                <span className="text-[10px] text-zinc-600">|</span>
                <button onClick={clearAllAccounts} className="text-[10px] text-zinc-400 hover:text-white">Clear</button>
              </div>
            </div>
              ...
              {accounts.map(acc => (
                ...
                >
                  <div className="flex items-center gap-3">
                    <img src={acc.avatarUrl} crossOrigin="anonymous" className="w-6 h-6 rounded-full object-cover border border-white/15" alt="" />
                    <div>
                  ...
                  {selectedAccounts.includes(acc._id) && (
                    <Check className="w-3.5 h-3.5 text-[#7831d6]" />
                  )}
                </div>
              ))}
              {accounts.length === 0 && (
        ...
        {user && (
          <div className="flex items-center gap-3 border-l border-white/10 pl-6">
            ...
            <div className="hidden md:block">
              ...
              {isAdminViewingUser && (
                <p className="mt-1 text-[9px] font-semibold uppercase tracking-wider text-[#c4b5fd]">
            ...
            {isAdminViewingUser && (
              <button
                ...
                className="inline-flex items-center gap-1.5 rounded-lg border border-white/10 bg-white/5 px-2.5 py-1.5 text-[10px] font-semibold text-white transition hover:bg-white/10"
            ...
            <button
              onClick={logout}
              className="p-1 hover:text-[#7831d6] text-zinc-400 rounded-lg transition-all"
              title="Logout"
            >
