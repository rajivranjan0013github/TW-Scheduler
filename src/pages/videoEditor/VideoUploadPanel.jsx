      ...
      <div className="flex items-center justify-between pb-2 border-b border-gray-100">
        ...
        <label className="flex items-center gap-1.5 cursor-pointer select-none">
          <input
            type="checkbox"
            checked={isDualVideo}
            onChange={(e) => onToggleDualVideo?.(e.target.checked)}
            className="hidden"
          />
          <div className={`relative w-7 h-4 rounded-full transition-colors ${isDualVideo ? 'bg-[#ff5500]' : 'bg-gray-200'}`}>
          ...
          <span className="text-[9px] font-bold text-gray-500 uppercase tracking-wider">Dual Mode</span>
        </label>
      </div>
      ...
      <div className="space-y-3">
        <div>
          <label className="block text-[10px] font-bold text-gray-400 uppercase mb-1">
            {isDualVideo ? 'First Video (Clips Starts)' : 'Video File'}
          </label>
          <button
            type="button"
