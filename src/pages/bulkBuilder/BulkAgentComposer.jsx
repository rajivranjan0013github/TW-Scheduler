            ...
            <div className={`space-y-2.5 px-4 py-3.5 ${messages.length === 0 && !isPlanning && !plan ? 'flex flex-1 flex-col items-center justify-center' : ''}`}>
              ...
              {messages.slice(-8).map((message) => (
                <div key={message.id} className={`flex ${message.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[85%] whitespace-pre-wrap rounded-2xl px-3.5 py-2.5 text-[11px] leading-relaxed shadow-sm ${message.role === 'user'
                    ? 'bg-gradient-to-r from-violet-600 to-[#7831d6] text-white rounded-br-sm'
                    : 'border border-white/[0.08] bg-white/[0.04] text-zinc-200 rounded-bl-sm'
                  }`}>
                    {message.content}
                  </div>
                </div>
              ...
              {isPlanning && (
                <div className="flex items-center gap-2.5 rounded-xl border border-violet-500/20 bg-violet-500/5 px-3 py-2 text-[10px] font-medium text-violet-300">
                  <Loader2 className="h-3.5 w-3.5 animate-spin text-violet-400" />
                  <span>Checking board, media pools, and planning tasks…</span>
                </div>
              )}
