...
const CalendarView = ({ selectedAccounts, composerOnly = false }) => {
  ...
  return (
    <div className="py-2 px-0 bg-[#f5f5f7] h-screen text-[#1d1d1f] font-sans flex flex-col overflow-hidden">
                        ...
                        >
                          ...
                          <div className="flex-shrink-0 ml-2">
                            {isCarousel ? (
                              ...
                              const childSubs = folders.filter(f => (normalizeFolderId(f.parentFolderId) || 'root') === folder._id);
      ...
      {!showComposer && (
        <section className="relative flex-1 min-h-0 bg-black flex flex-col overflow-hidden">
          {isLoadingCalendar && (
          ...
          {/* Clean Filter Bar */}
          <div className="border-b border-white/[0.08] bg-[#0a0a0a] px-4 py-2.5 flex-shrink-0">
              ...
              <div className="flex min-w-0 flex-wrap items-center gap-3">
	                  ...
	                  <button
	                    ...
	                    className="flex items-center gap-1.5 h-8 rounded-lg border border-white/10 bg-white/5 px-3 text-[12px] font-medium text-white hover:bg-white/10 transition-colors"
	                  >
	                    <svg className="h-3.5 w-3.5 text-zinc-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M22 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>
	                        ...
	                        {filteredCalendarAccountOptions.map((option) => {
	                          ...
	                          return (
	                            ...
	                            >
	                              ...
	                              <span className="min-w-0 flex-1 truncate text-[13px] font-semibold text-white">@{option.label}</span>
	                              <span className="flex-shrink-0 text-[9px] font-bold uppercase text-zinc-500">{option.platform}</span>
	                            </button>
	                          );
	                        })}
	                        {filteredCalendarAccountOptions.length === 0 && (
	                          <div className="px-3 py-3 text-[12px] font-medium text-zinc-500 text-center">
                    ...
                    <div className="absolute left-0 top-9 z-[9999] w-[24rem] max-w-[calc(100vw-2rem)] overflow-hidden rounded-2xl border border-white/[0.08] bg-[#141417]/95 shadow-2xl backdrop-blur-2xl">
                      ...
                      <div className="max-h-64 overflow-y-auto border-t border-white/[0.08] py-1 custom-scrollbar">
                        ...
                        {filteredCalendarHandlerOptions.length === 0 && (
                          <div className="px-3 py-3 text-center text-[12px] font-medium text-zinc-500">
                ...
                <div className="flex items-center gap-1.5 h-8 rounded-lg border border-white/10 bg-white/5 px-3">
          ...
          <div className="flex-1 min-h-0 flex overflow-hidden">
            {/* Calendar Grid */}
            <div className="flex-1 min-h-0 flex flex-col overflow-hidden border-r border-white/[0.08] relative">
              {/* Calendar Cells */}
