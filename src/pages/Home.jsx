...
export const Home = () => (
  <div className="h-screen overflow-y-auto bg-black text-white font-sans selection:bg-[#7831d6] selection:text-white">
    <header className="sticky top-0 z-20 border-b border-white/10 bg-black/80 backdrop-blur-md px-5 py-4">
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <div className="flex h-9 w-9 items-center justify-center rounded-lg border border-[#7831d6]/40 bg-[#7831d6]/20 text-[#c4b5fd] shadow-[0_0_16px_rgba(120,49,214,0.35)]">
        ...
        <Link
          to="/login"
          className="inline-flex items-center gap-2 rounded-lg bg-[#7831d6] px-4 py-2 text-xs font-semibold text-white transition hover:bg-[#6825bc] shadow-lg shadow-[#7831d6]/25"
        >
          Sign in
    ...
    <main>
      <section className="relative overflow-hidden bg-black">
        ...
        <div className="absolute -top-40 left-1/2 -translate-x-1/2 w-[1000px] h-[500px] bg-[#7831d6]/15 blur-[120px] pointer-events-none rounded-full" />
        ...
        <div className="relative mx-auto grid min-h-[calc(100vh-74px)] max-w-7xl grid-cols-1 items-center gap-10 px-5 py-12 lg:grid-cols-[0.94fr_1.06fr] lg:py-16">
          <div className="max-w-2xl">
            <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-3 py-1.5 text-[11px] font-semibold uppercase tracking-wider text-zinc-300">
            ...
            <div className="mt-8 rounded-xl border border-white/10 bg-[#0a0a0a] p-4 shadow-2xl sm:inline-flex sm:items-center sm:gap-5">
              ...
              <Link
                to="/login"
                className="inline-flex items-center justify-center rounded-lg bg-[#7831d6] px-5 py-2.5 text-sm font-semibold text-white transition hover:bg-[#6825bc] shadow-lg shadow-[#7831d6]/30"
              >
                Open login
              </Link>
            </div>
                        ...
                        <div key={post.title} className="flex items-center justify-between gap-3 rounded-lg bg-black border border-white/10 p-3 transition hover:bg-white/[0.04]">
                          ...
                          <span className={`rounded-md border px-2 py-1 text-[10px] font-bold ${
                            post.status === 'Ready'
                              ...
                              : post.status === 'Queued'
                              ? 'border-[#7831d6]/30 bg-[#7831d6]/20 text-[#c4b5fd]'
                    ...
                    <div className="mt-5 flex h-44 items-end gap-2">
                      {[36, 54, 48, 72, 62, 88, 96].map((height, index) => (
                        <div key={height} className="flex flex-1 flex-col items-center gap-2">
                          <div
                            className="w-full rounded-t-md bg-[#7831d6]"
      ...
      <section className="border-t border-white/10 bg-black px-5 py-12">
        <div className="mx-auto grid max-w-7xl gap-4 md:grid-cols-3">
          {features.map((feature) => (
            <div key={feature.title} className="rounded-xl border border-white/10 bg-[#0a0a0a] p-6 transition hover:border-white/20 hover:bg-white/[0.03]">
    ...
    <footer className="border-t border-white/10 bg-black px-5 py-10 text-white">
      <div className="mx-auto grid max-w-7xl gap-8 md:grid-cols-[1.4fr_1fr_1fr]">
        <div>
          <div className="flex items-center gap-3">
            <div className="flex h-9 w-9 items-center justify-center rounded-lg border border-[#7831d6]/40 bg-[#7831d6]/20 text-[#c4b5fd]">
              <Sparkles className="h-4 w-4" />
            </div>
            <div>
              <p className="m-0 text-base font-bold tracking-tight text-white">EasyPost</p>
