...
export const YoutubeCallback = () => {
  ...
  const [statusMessage, setStatusMessage] = useState('Securely exchanging credentials with YouTube to connect your channel...');
  ...
  const exchangeToken = useCallback(async () => {
    try {
      ...
      if (response.ok) {
        ...
        const channelName = account?.name || 'Your YouTube channel';
        const successMsg = `${channelName} is ready for scheduled uploads.`;
        ...
        setStatus('error');
        setStatusMessage(data.message || 'YouTube authorization failed.');
      }
    } catch (err) {
      console.error('Error in YouTube OAuth token exchange:', err);
      setStatus('error');
      setStatusMessage('Network error completing YouTube authentication flow.');
    }
  ...
  useEffect(() => {
    if (error) {
      setStatus('error');
      setStatusMessage(`YouTube authorization was cancelled or failed: ${error}`);
      return;
    }
      ...
      <div className="w-full max-w-sm rounded-2xl border border-white/10 bg-[#0c0c0e] p-6 shadow-2xl text-center space-y-5 animate-in fade-in zoom-in-95 duration-200">
        {status === 'success' && connectedAccount ? (
          <div className="space-y-4 animate-in zoom-in-95 duration-200">
            ...
            <div>
              ...
              <h3 className="text-base font-bold tracking-tight text-white m-0 truncate">
                {connectedAccount.name || connectedAccount.displayName || 'YouTube Channel'}
              </h3>
          ...
          <>
            <div className="flex justify-center">
              <div className="relative">
                <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-red-600 shadow-lg shadow-red-600/25">
                  <PlatformIcon platform="youtube" className="h-7 w-7 text-white" />
                </div>
                {status === 'processing' && (
                  <span className="absolute -bottom-1 -right-1 h-5 w-5 animate-spin rounded-full border-2 border-red-500 border-t-transparent bg-black" />
                )}
                {status === 'success' && (
        ...
        {status === 'processing' && (
          <div className="flex justify-center pt-1">
            <span className="text-[11px] font-medium text-zinc-500">Authorizing channel credentials...</span>
          </div>
        )}
