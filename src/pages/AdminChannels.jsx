...
export const AdminChannels = () => {
  ...
  const channelsQuery = useQuery({
    queryKey: ['admin-channels', channelEndpoint, channelQueryParam, adminViewUserId],
    queryFn: async () => {
  ...
  const allAccountsQuery = useQuery({
    ...
    staleTime: 2 * 60 * 1000,
    enabled: Boolean(user && activeConnectCampaignId),
  });
  ...
  const confirmRemoveChannel = async () => {
    ...
    const accountLabel = channelToRemove.displayName || channelToRemove.name || formatHandle(channelToRemove.handle || channelToRemove.requestedHandle || channelToRemove.username || 'Channel');
    ...
    try {
      ...
      setToast({
        type: 'success',
        message: `${accountLabel} was ${isWorkspaceDisconnect ? 'disconnected from workspace' : 'removed from campaign'} successfully.`,
      });
    } catch (err) {
  ...
  const otherUnlinkedAccounts = useMemo(() => {
    if (!activeConnectCampaignId) return [];
    const allAccounts = Array.isArray(allAccountsQuery.data) ? allAccountsQuery.data : [];
    const linkedAccountIds = new Set(
      normalizedChannels.map((ch) => getChannelAccountId(ch)).filter(Boolean)
    );
    return allAccounts.filter((acc) => !linkedAccountIds.has(acc._id));
  }, [allAccountsQuery.data, normalizedChannels, activeConnectCampaignId]);
  ...
  return (
    <div className="min-h-screen w-full bg-[#0c0c0e] p-4 lg:p-6 text-white space-y-6 font-sans antialiased pb-16">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-zinc-400 mt-1 m-0">
            Manage connected social accounts and assign external creators to your product campaigns.
          </p>
        </div>
      ...
      {/* Inline Form: Assign Channel to Creator */}
      {activeConnectCampaignId && (
        <form
      ...
      {otherUnlinkedAccounts.length > 0 && (
          ...
          <div className="w-full max-w-md rounded-2xl border border-white/10 bg-[#121215] p-6 shadow-2xl space-y-4">
            <div className="flex items-start gap-3.5">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="mt-1 text-xs text-zinc-400 m-0">
                  {(!activeConnectCampaignId || channelToRemove.isWorkspaceAccount)
                    ? 'This will completely disconnect the account from your workspace.'
                    : 'This will unlink this publishing channel from the current campaign.'}
                </p>
              </div>
            </div>
            ...
            <div className="rounded-xl border border-rose-500/20 bg-rose-500/[0.05] p-3.5 space-y-1.5 text-xs text-zinc-300 leading-relaxed">
              <p className="m-0 font-semibold text-rose-300">
                {(!activeConnectCampaignId || channelToRemove.isWorkspaceAccount) ? 'Workspace Disconnect Impact:' : 'Campaign Removal Impact:'}
              </p>
              <ul className="m-0 list-disc list-inside space-y-1 text-[11px] text-zinc-400 pl-0.5">
                {(!activeConnectCampaignId || channelToRemove.isWorkspaceAccount) ? (
                  <>
                    <li>Revokes stored API tokens with <span className="capitalize text-zinc-200">{channelToRemove.platform}</span>.</li>
                    <li>Removes the channel from all campaigns across the entire workspace.</li>
                    <li>Scheduled posts targeted to this account will be paused or fail.</li>
                  </>
                ) : (
                  <>
                    <li>Unlinks the channel from the currently selected campaign.</li>
                    <li>The social account remains safely connected in your workspace for other campaigns.</li>
                    <li>You can re-link it anytime with 1-click from the workspace accounts pool below.</li>
                  </>
                )}
              </ul>
            </div>
          ...
          <div className="w-full max-w-sm rounded-2xl border border-white/10 bg-[#121215] p-6 shadow-2xl text-center space-y-4">
            ...
            <div>
              ...
              <p className="mt-2 text-xs text-zinc-400 leading-relaxed m-0">
                Your <span className="capitalize text-zinc-200">{connectedDialogAccount.platform === 'youtube' ? 'YouTube' : connectedDialogAccount.platform}</span> account is now linked and verified. You can now publish and track posts across this channel.
              </p>
            </div>
